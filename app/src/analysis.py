...
class Analysis:
    def __init__(self, argies, api_entry=True) -> None:
        self.a = argies
        if api_entry:
            self.a["input_file"] = f"{argies['ExpDir']}/PosCounts.csv"
    ...
    def add_probelength(self):
        ...
        try:
            probelengths = pd.read_csv(self.a["Probes"], dtype={'target_len':int})
        except:
            stoperr(f'Failed to read probe information. Is {self.a["Probes"]} a valid CSV file?')
        probelengths_mod = self.add_probetype(probelengths)
        self.df = self.df.merge(probelengths_mod, left_on='target_id', right_on='target_id', how='left')
    ...
    def add_probetype(self, pdf):
        ...
        pdf['genename'] = pdf.target_id.apply(lambda x: x.replace('_','-').split('-')[0])
        pdf.loc[pdf.target_id=='roseolovirus_allrecords_cluster_1', 'genename'] = 'HHV7_roseolovirus_allrecords_cluster_1'
        pdf.loc[pdf.target_id=='roseolovirus_allrecords_cluster_2', 'genename'] = 'HHV6_roseolovirus_allrecords_cluster_2'

        '''More precise definition for the different virus types'''
        pdf.loc[pdf.genename=='enterovirus', 'genename'] = pdf.loc[pdf.genename=='enterovirus'].target_id.apply(lambda x: '_'.join(x.replace('_','-').split('-')[:2]))
        pdf.loc[pdf.genename=='Coronaviridae', 'genename'] = pdf.loc[pdf.genename=='Coronaviridae'].target_id.apply(lambda x: '_'.join(x.replace('_','-').split('-')[:3]))
        pdf.loc[pdf.genename=='Adenoviridae', 'genename'] = pdf.loc[pdf.genename=='Adenoviridae'].target_id.apply(lambda x: '_'.join(x.replace('_','-').split('-')[:2]))
        pdf.loc[pdf.genename=='Flaviviridae', 'genename'] = pdf.loc[pdf.genename=='Flaviviridae'].target_id.apply(lambda x: '_'.join(x.replace('_','-').split('-')[:2]))
        pdf.loc[pdf.genename=='Influenza', 'genename'] = pdf.loc[pdf.genename=='Influenza'].target_id.apply(lambda x: '_'.join(x.replace('_','-').split('-')[:2]))
        pdf.loc[pdf.genename=='Paramyxoviridae', 'genename'] = pdf.loc[pdf.genename=='Paramyxoviridae'].target_id.apply(lambda x: '_'.join(x.replace('_','-').split('-')[:2]))
        pdf.loc[pdf.genename=='Parvoviridae', 'genename'] = pdf.loc[pdf.genename=='Parvoviridae'].target_id.apply(lambda x: '_'.join(x.replace('_','-').split('-')[:2]))
        ...
        pdf.loc[pdf.genename.str.startswith('BACT'), 'probetype'] = pdf.loc[pdf.genename.str.startswith('BACT')].target_id.apply(_pat_search)
        ...
        pdf.loc[(pdf.target_id.apply(lambda x: 'pneumoniae' in x)) & (pdf.probetype.isin(('Streptococcus_pneumoniae', 'Streptococcus_pseudopneumoniae', 'Streptococcus_mitis', 'Streptococcus_oralis'))), 'probetype'] = 'Streptococcus_pneumoniae'
        '''Streptococcus_agalactiae and Streptococcus_pyogenes cross-map as well, aggregate them'''
        pdf.loc[(pdf.target_id.apply(lambda x: 'pyogenes' in x or 'agalactiae' in x)) & (pdf.probetype.isin(('Streptococcus_pyogenes', 'Streptococcus_agalactiae'))), 'probetype'] = 'Streptococcus_agalactiae_pyogenes'
        '''Enterobacteriacae are not distinguishable at this level so group them all'''
        pdf.loc[pdf.probetype.apply(lambda x: x.startswith('Escherichia') or x.startswith('Klebsiella') or x.startswith('Enterobacter')), 'probetype'] = 'Enterobacteriaceae'
        loginfo(f'Organism and gene summary: {pdf.probetype.nunique()} organisms, up to {pdf.groupby("probetype").genename.nunique().max()} genes each.')
        return pdf
    ...
    def reassign_dups(self):
        ...
        grouped_prededup_mapped_reads = self.df.groupby(['sampleid','probetype']).n.sum().unstack().fillna(0)
        self.df.sort_values(['target_id', 'startpos', 'maplen', 'n'], ascending=True, inplace=True)
        self.df['n'] = self.df.groupby(['target_id', 'startpos', 'maplen']).n.cumsum()
        nrows0 = len(self.df)
        duprows = self.df.duplicated(['target_id', 'startpos', 'maplen'], keep='last')
        loginfo(f'Saving {self.a["ExpName"]}_reads_to_drop.csv')
        self.df[duprows][['sampleid', 'target_id', 'startpos', 'maplen']].to_csv(f'{self.a["ExpDir"]}/{self.a["ExpName"]}_reads_to_drop.csv', index=False)
        self.df = self.df[~duprows]
        loginfo(f'Kept {len(self.df)} of {nrows0} rows ({float(len(self.df))/nrows0}).')
        ttl_dup_mapped_reads = self.df.groupby('sampleid').n.sum()
        grouped_dup_mapped_reads = self.df.groupby(['sampleid','probetype']).n.sum().unstack().fillna(0)
        try:
        ...
        loginfo(f'Saving {self.a["ExpName"]}_duprate.csv and {self.a["ExpName"]}_duprate_by_probetype.csv.')
        duprate_ttl.to_csv(f'{self.a["ExpDir"]}/{self.a["ExpName"]}_duprate.csv',header=True)
        duprate_by_probetype.to_csv(f'{self.a["ExpDir"]}/{self.a["ExpName"]}_duprate_by_probetype.csv')
    ...
    def add_depth(self, probelengths):
        ...
        if self.a["DepthInf"]:
            loginfo(f'Reading read depth information from {self.a["DepthInf"]}.')
            try:
        ...
        if not depth:
            metrics = {}
            odir = f'{self.a["ExpDir"]}/Depth_{self.a["ExpName"]}'
            if not os.path.isdir(odir):
                try:
                    os.mkdir(odir)
                except OSError:
                    odir=os.getcwd()
                    logerr(f'Cannot create output directory {odir} for saving depth information. Proceeding with current working directory.')

            loginfo('INFO: Calculating read depth statistics for all probes, for all samples. This is a slow step.')
            for (sampleid, probetype), g in self.df.groupby(['sampleid', 'probetype']):
                ...
                loginfo(f'Mean depth (deduplicated) for {probetype}: {D1.mean()}')
                '''Amplification rate calculations use the unpadded (mapped) number of sites as the denominator'''
                loginfo(f'Mean amplification ratio for {probetype}: {amprate.mean()}')
                ...
                if D1.mean() >= 0.1:
                    ...
                    try:
                        ...
                        pass
                    plt.savefig(f'{odir}/{sampleid}/{probetype}-{sampleid}.pdf')
            ...
            '''Save and log'''
            loginfo(f'Saving {self.a["ExpDir"]}/{self.a["ExpName"]}_depth.csv.')
            depth.to_csv(f'{self.a["ExpDir"]}/{self.a["ExpName"]}_depth.csv', index=False)
            loginfo(f'Mean read depth per sample: \n{depth.groupby("sampleid").depth_mean.mean().to_string()}')
        return depth
    ...
    def add_clin(self, req_cols_samples = ['sampleid', 'pt', 'rawreadnum'], req_cols_clin = ['pt', 'clin_int']):
        ...
        if set(req_cols_samples) & set(samples.columns) != set(req_cols_samples):
            stoperr(f'Samples file {self.a["Samples"]} must contain at least the following columns: {req_cols_samples}')
        if self.a["Clin"]:
            ...
            if 'pt' not in clin.columns:
                stoperr(f'Clin file {self.a["Clin"]} must contain at least the following columns: {req_cols_clin}')
            if 'sampleid' in clin.columns:
                logerr(f'Ignoring "sampleid" column from clinical info file {self.a["Clin"]}.')
        ...
        loginfo(f'Saving {self.a["ExpDir"]}/{self.a["ExpName"]}_depth_with_clin.csv.')
        cdf.to_csv(f'{self.a["ExpDir"]}/{self.a["ExpName"]}_depth_with_clin.csv', index=False)
        return cdf
    ...
    def save_tophits(self, depth):
        ...
        tophits.to_csv(f'{self.a["ExpDir"]}/{self.a["ExpName"]}_tophit.csv', index=False)
        loginfo(f'Saved top hits to {self.a["ExpDir"]}/{self.a["ExpName"]}_tophits.csv')
    ...
    def main(self):
        ...
        self.df = self.df.merge(depth, on=['sampleid', 'probetype'], how='left')
        self.df.to_csv(f'{self.a["ExpDir"]}/{self.a["ExpName"]}_fullself.df.csv.gz', index=False, compression='gzip')
        loginfo(f'Finished. Saved final data frame as {self.a["ExpDir"]}/{self.a["ExpName"]}_fullself.df.csv.gz')
